...
# 각 노드를 초기화
builder.add_node("search_menu", search_menu_subgraph)
builder.add_node("search_wine", search_wine_subgraph)
builder.add_node("search_web", search_web_subgraph)
builder.add_node("filter_documents", filter_documents_subgraph)
...
# png_data = search_graph.get_graph().draw_mermaid_png()

# with open("stategraph.png", "wb") as f:
#     f.write(png_data)

result = search_graph.invoke({"question": "스테이크 메가 있으면 추천해주세요. 그고 어울리는 와인도 소개해주세요."})
pprint(result)