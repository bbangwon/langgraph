...
def reduce_unique_documnents(left: list | None, right: list | None) -> list:
    if not left:
        left = []
    if not right:
        right = []
    # 중복 제거: set을 사용하여 문서를 제거하고 다시 list로 변환
    return list(set(left + right))

# 상태 정의 (documents 필드 포함)
class CustomReducerState(TypedDict):
    query: str
    documents: Annotated[List[str], reduce_unique_documnents]
